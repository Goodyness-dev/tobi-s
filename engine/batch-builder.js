...
const baseDocumentsDir = fs.existsSync(path.resolve(__dirname, '..', 'service-biz-master-template'))
  ? path.resolve(__dirname, '..')
  : path.resolve(__dirname, '../..');

// 1. Parse CLI Arguments
const args = process.argv.slice(2);
let targetsFile = fs.existsSync(path.resolve(process.cwd(), 'targets.txt'))
  ? path.resolve(process.cwd(), 'targets.txt')
  : path.resolve(__dirname, 'targets.txt');
let concurrency = 3;
