    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-8 text-center">
        ...
        <button 
          ...
          className="px-6 py-3 rounded-full bg-teal-600 text-white font-semibold"
        >
          Return Home
        </button>
      </div>
  ...
  return (
    <div className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-xs font-medium text-slate-500 dark:text-slate-400 mb-6">
        ...
        <span>/</span>
        <button onClick={() => onNavigate('procedures')} className="hover:text-teal-600 transition">
          Surgical Procedures
        </button>
        <span>/</span>
      ...
      <div className="mb-10 text-center sm:text-left">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-teal-50 dark:bg-teal-950/70 border border-teal-200 dark:border-teal-800/60 text-teal-700 dark:text-teal-300 text-xs font-bold uppercase tracking-wider mb-3">
          <span>{procedure.category}</span>
          <span>•</span>
          <span>Easton, PA Surgical Center</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-16 items-start">
          ...
          <div className="mt-10 card-thick bg-white dark:bg-[#0f172a] rounded-3xl border-2 border-slate-200/90 dark:border-slate-800/90 p-8 sm:p-10 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {procedure.advantages.map((adv, idx) => (
                <div key={idx} className="flex items-start gap-3 p-3.5 rounded-2xl bg-teal-50/60 dark:bg-teal-950/30 border border-teal-100 dark:border-teal-900/40 text-sm font-medium text-slate-800 dark:text-slate-200">
                  <svg className="w-5 h-5 text-teal-600 dark:text-teal-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* Right Column: Sticky Consultation Card & Surgeon Assurance */}
        <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-28">
          <div className="card-thick bg-gradient-to-b from-teal-900 to-slate-950 text-white rounded-3xl p-7 sm:p-8 border-2 border-teal-700/50 shadow-xl">
            <div className="inline-block px-3 py-1 rounded-full bg-teal-800/80 text-teal-200 text-xs font-semibold mb-3 border border-teal-600/40">
              Direct Consultation
            </div>
            ...
            <p className="text-teal-100/90 text-sm leading-relaxed mb-6">
              Receive a complete clinical diagnosis, 3D CBCT digital scan evaluation, and tailored sedation plan in our Easton surgical suite.
            </p>

            <div className="space-y-3 mb-6">
              <button
                onClick={() => onOpenConsultation(procedure.id)}
                className="w-full py-4 rounded-2xl bg-teal-400 hover:bg-teal-300 text-slate-950 font-bold text-sm tracking-wide shadow-lg shadow-teal-900/50 active:scale-98 transition flex items-center justify-center gap-2"
              >
            ...
            <div className="pt-4 border-t border-teal-800/80 text-xs text-teal-200/80 space-y-1.5">
              <p className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-teal-400" />
                In-network with Delta, Aetna, Cigna, Blue Cross
              </p>
              <p className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-teal-400" />
          ...
          <div className="card-thick bg-white dark:bg-[#0f172a] rounded-3xl border-2 border-slate-200/90 dark:border-slate-800/90 p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-2xl bg-teal-100 dark:bg-teal-950 text-teal-700 dark:text-teal-300 flex items-center justify-center shrink-0">
              ...
              <div>
                <h4 className="font-bold text-sm text-slate-900 dark:text-white">
                  Zero Surgical Memory & Fear
                </h4>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Certified IV Sleep Anesthesia
                </p>
              </div>
            </div>
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              Our Easton facility is equipped with continuous hospital-grade monitors. Our surgical assistants are DAANCE certified so you rest with total peace of mind.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-3">
            {procedure.candidates.map((c, i) => (
              <div key={i} className="flex items-start gap-3 text-sm text-slate-700 dark:text-slate-300">
                <span className="w-5 h-5 rounded-full bg-slate-100 dark:bg-slate-800 text-teal-600 dark:text-teal-400 flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">
                  ✓
                </span>
                <span>{c}</span>
              </div>
      ...
      {/* Other Procedures Carousel/Selector */}
      <div className="card-thick bg-slate-50 dark:bg-[#0f172a] rounded-3xl border-2 border-slate-200/90 dark:border-slate-800/90 p-8 text-center">
        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">
          Explore Other Specialty Procedures
        </h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
          Dedicated 3D clinical guides for all procedures performed at Milano & Mazza
        </p>
        <div className="flex flex-wrap justify-center gap-2.5">
          {PROCEDURES.filter(p => p.id !== procedure.id).map(p => (
            <button
              ...
              className="px-4 py-2 rounded-full bg-white dark:bg-slate-900 hover:bg-teal-50 dark:hover:bg-teal-950/50 text-slate-800 dark:text-slate-200 hover:text-teal-700 dark:hover:text-teal-300 border border-slate-200 dark:border-slate-800 font-medium text-xs transition shadow-sm"
            >
              {p.title} →
            </button>
          ))}
